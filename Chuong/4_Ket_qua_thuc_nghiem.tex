...
Đối với đề tài game, sinh viên cần đưa ra các screenshot liên quan đến các chức năng khác nhau của trò chơi (ví dụ combat, tương tác với NPC, di chuyển, bị tấn công, sử dụng kỹ năng...) chứ \textbf{không phải chỉ đưa ra 1 màn hình "in game" duy nhất}. Các màn hình có giao diện quá giống nhau (ví dụ các màn hình inventory, equipment...) thì có thể bỏ qua. 

Quan trọng nhất là, sinh viên phải hình dung viết mục này làm sao để người đọc luận văn của các bạn, có thể không cần down game của bạn về hay xem video quảng cáo vẫn có thể hình dung ra được trò chơi của các bạn như thế nào.
...
\begin{itemize}
    \item Where is it deployed? 
    \item How can they recreate it? 
    \item What information is needed to get it started? Do not, however, include an passwords in this document!
    \item What systems are required to run the game? 
    \item If it is a game that runs on local devices, deployment includes how to rebuild it and where it needs to be stored for users to be able to download it
    \item What configurations and settings are required to get the game working?
\end{itemize}
